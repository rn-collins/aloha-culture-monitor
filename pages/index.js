  ...
  return(<>
    ...
    <div style={{minHeight:'100vh',display:'flex',flexDirection:'column',background:BG}}>
      ...
      <main style={{flex:1,maxWidth:1280,margin:'0 auto',padding:'48px 40px',width:'100%'}}>
        ...
        {loading&&(<div style={{display:'flex',alignItems:'center',gap:12,padding:'60px 0',color:MU,fontSize:14}}><div style={{width:16,height:16,borderRadius:'50%',background:G,animation:'pulse 1.2s ease-in-out infinite'}}/>Loading live signals from Wikipedia, Reddit & Google Trends — this takes about 3 seconds...<style>{`@keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.4;transform:scale(.85)}}`}</style></div>)}
